...
import type { ${modelName} } from '@/04_types${group ? `/${convertNaming(group, 'KebabSingular')}` : ''}/${convertNaming(
      table,
      'KebabSingular',
    )}';
...
// Create the store
const use${modelName}Store = create<${modelName}StoreProps>(set => ({
  selected${modelName}: null,
  setSelected${modelName}: ${camelModel} =>
    set({ selected${modelName}: ${camelModel} }),
}));

export default use${modelName}Store;
`;
  };

  const code = generateStore();
